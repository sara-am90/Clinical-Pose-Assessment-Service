...
class RomeEngine:
    ...
    @staticmethod
    def calculate_shoulder_abduction(keypoints, side="right"):
        """
        Calculates Shoulder Abduction Angle (Shoulder -> Elbow vs Vertical Torso).
        COCO keypoints:
        ...
        if s_idx not in keypoints or e_idx not in keypoints or h_idx not in keypoints:
            return 0.0

        shoulder = np.array(keypoints[s_idx][:2])
        elbow = np.array(keypoints[e_idx][:2])
        hip = np.array(keypoints[h_idx][:2])

        # Vector 1: Torso line (Hip -> Shoulder)
        torso_vec = shoulder - hip
        # Vector 2: Upper arm (Shoulder -> Elbow)
        arm_vec = elbow - shoulder

        # Calculate angle between torso vector and arm vector
        unit_torso = torso_vec / (np.linalg.norm(torso_vec) + 1e-6)
        unit_arm = arm_vec / (np.linalg.norm(arm_vec) + 1e-6)

        dot_product = np.clip(np.dot(unit_torso, unit_arm), -1.0, 1.0)
